...
export default function SRRegister({ searchFilter, clearSearchFilter }) {
  ...
  const handleSMChange = (smId) => {
    setModal(m => {
      const updatedData = { ...m.data, sm_id: smId };
      if (smId) {
        const selectedSM = sms.find(s => s.id === smId);
        if (selectedSM) {
          updatedData.ssm_id = selectedSM.ssm_id || null;
          updatedData.am_id = selectedSM.am_id || null;
        }
      }
      return { ...m, data: updatedData };
    });
    setErrors(e => ({ ...e, sm_id: undefined, ssm_id: undefined, am_id: undefined }));
  };
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="form-group">
              <label className="form-label required">SM</label>
              <div className="relative">
                <SearchableDropdown id="sr-sm" options={smOpts} value={modal.data.sm_id} onChange={handleSMChange} placeholder="Select SM…"/>
              </div>
              {errors.sm_id && <span className="text-error text-xs mt-1">{errors.sm_id}</span>}
            </div>

            <div className="form-group">
              <label className="form-label required">SSM</label>
              <div className="relative">
                <SearchableDropdown id="sr-ssm" options={ssmOpts} value={modal.data.ssm_id} onChange={handleSSMChange} placeholder="Select SSM…"/>
              </div>
